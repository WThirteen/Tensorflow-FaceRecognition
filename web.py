...
register_interface = gr.Interface(
    fn=face_register,
    inputs=gr.Image(type="numpy", label="上传注册人脸图像"),
    outputs=gr.Textbox(label="注册结果"),
    title="人脸注册",
    description="请上传包含人脸的图像进行注册"
)
...
recognition_interface = gr.Interface(
    fn=face_recognition,
    inputs=gr.Image(type="numpy", label="上传待识别人脸图像"),
    outputs=gr.Image(label="识别结果"),
    title="人脸识别",
    description="请上传待识别的人脸图像"
)
